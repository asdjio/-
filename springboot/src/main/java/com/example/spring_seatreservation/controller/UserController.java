...
public class UserController {
    ...
    public Map<String, Object> toSigned(@RequestBody Map<String, Object> map) {
        ...
        if (number==9999999) {
            //dynamicTask.add(new MyTask(ReservationCode.TIME_BEGAN + "-" + map.get("sid"),
                    //currentTimeMillis, () -> {
            dynamicTask.stop(ReservationCode.UNSIGNED + "-" + reservation.get("sid").toString());                userMapper.updateReservation(ReservationCode.FINISH, map.get("rid"));
            userMapper.updateReservation(ReservationCode.FINISH, reservation.get("rid"));
            userMapper.updateSeat(SeatCode.CAN_USE, map.get("sid"));
            //}));
            return new R().ok().builder();
        }
        //预约签到
        else if (state.equals(ReservationCode.TIME_BEGAN)) {
